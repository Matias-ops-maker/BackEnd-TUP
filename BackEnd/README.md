...
## 🧩 Cómo ejecutar cuando BACKEND y FRONTEND estén en repos separados

El backend y el frontend pueden vivir en repositorios distintos. Aquí están los pasos para levantar ambos en tu máquina local y configurarlos para que se conecten.

1) Clonar repositorios en carpetas separadas (por ejemplo `repuestos-backend` y `repuestos-frontend`).

2) Backend: instalar dependencias, configurar `.env` y levantar servidor
```powershell
cd repuestos-backend
npm install
# Crear archivo .env copiando .env.example y rellenando los valores
# Por ejemplo:
Set-Content -Path .env -Value "JWT_SECRET=tu_jwt_secret`nMP_ACCESS_TOKEN=tu_access_token_sandbox`nMP_PUBLIC_KEY=tu_public_key_sandbox`nPORT=4000"
# (Opcional) migrar y seed
npm run migrate
npm run seed
npm start
```

3) Frontend: configurar la URL del backend (en `.env`) y levantar
```powershell
cd repuestos-frontend
Set-Content -Path .env -Value "VITE_API_URL=http://localhost:4000/api`nVITE_MP_PUBLIC_KEY=tu_public_key_de_mercadopago"
npm install
npm run dev
```

4) CORS: si el backend y frontend están en dominios/puertos distintos, asegúrate que en `src/app.js` del backend la configuración de CORS incluya la URL del frontend. Ejemplo:
```javascript
app.use(cors({ origin: process.env.FRONTEND_URL || 'http://localhost:5173', credentials: true }));
```

5) Webhooks y MercadoPago: si usas MercadoPago en ambiente de pruebas, setea en el panel de MercadoPago la URL del webhook apuntando al backend (ejemplo `http://your-public-domain.com/api/payments/webhook`). Si desarrollas localmente y necesitas recibir webhooks, usa una herramienta como `ngrok` para exponer temporalmente tu backend.

6) Variables y secretos: nunca subas `.env` al repo. Incluye un `.env.example` con las variables necesarias. Añade `database.sqlite` y `node_modules/` a `.gitignore` si no quieres subir la DB ni dependencias.

Si quieres, puedo crear aquí los archivos `.env.example` y una versión mínima de `.gitignore` para cada carpeta (FRONTEND y BACKEND). ¿Los genero ahora? 
